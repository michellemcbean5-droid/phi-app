...
def get_llm(temperature=0.1):
    """Get LLM with mock fallback for development without API keys."""
    ...
    # Check for valid API keys - must be real keys, not placeholders
    def is_valid_key(key):
        if not key or len(key) < 30:
            return False
        placeholder_patterns = ["your", "here", "xxx", "...", "test", "demo", "placeholder", "example"]
        key_lower = key.lower()
        return not any(pattern in key_lower for pattern in placeholder_patterns)
    
    valid_keys = [k for k in api_keys if is_valid_key(k)]
    
    if not valid_keys:
        # Return mock LLM for testing without valid API keys
        mock_llm = MagicMock()
        mock_llm.invoke = lambda prompt: "Mock response for testing"
        mock_llm.bind = lambda **kwargs: mock_llm
        mock_llm.with_structured_output = lambda schema: mock_llm
        mock_llm.with_config = lambda **kwargs: mock_llm
        return mock_llm
...
_llm_persuasive = get_llm(temperature=0.7)
